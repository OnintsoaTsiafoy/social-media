...
Configuration ajoutée : `RAG_MIN_SIMILARITY=0.86`, transmis à Express dans Compose. Si un `.env` existant fixe encore 0.82, modifier cette valeur puis recréer l’API. Le modèle est fixé à `intfloat/multilingual-e5-small`, dimension 384, avec préfixes `query:` et `passage:`, pooling moyen et normalisation. Le téléchargement initial est placé dans le volume `embedding_models`. Aucune clé d’embedding n’est requise ; le modèle fonctionne ensuite localement. Changer le modèle exige de réindexer documents et exemples ; changer sa dimension exige aussi une migration.
...
Le Top K documentaire est plafonné à 10 (5 pour la génération). La similarité cosinus minimale est 0,86 par défaut. Les index HNSW sont présents ; la recherche documentaire courante calcule exactement les distances sur le sous-ensemble autorisé matérialisé pour éviter qu’un filtrage après HNSW ne perde les résultats de petites marques.

La confiance documentaire est la meilleure similarité retrouvée, ou zéro. Elle mesure la proximité et **n’est pas une probabilité de véracité**. L’écran distingue élevée (≥ 0,85), moyenne (≥ 0,70) et faible. Le seuil doit être recalibré sur des documents représentatifs. Le test Android a confirmé qu’un score de 0,84 pouvait associer une question sur les horaires à une politique de remboursement : ce résultat est désormais exclu.

`python -m evaluation.calibrate` mesure 75 couples synthétiques (15 questions × 5 documents), chaque document étant testé comme seule source disponible. À 0,82, 10/11 couples attendus sont retenus, avec 24/64 couples sans la source attendue également retenus ; à 0,86, ces nombres deviennent 8/11 et 2/64. Les deux derniers concernent des textes voisins sur les retours et remboursements. Le réglage privilégie l’abstention, avec une perte de rappel, notamment pour certaines questions multilingues. Ce petit corpus sert à la régression, pas à garantir l’absence de réponses hors sujet en production. Les mesures détaillées sont dans `evaluation/results/calibration.json`.
...
Vérification initiale du 14 septembre 2026 : 123 tests API réussis (intégration ignorée dans la suite unitaire), 1 scénario d’intégration PostgreSQL réussi, 181 tests IA réussis avec embeddings réels, 158 tests du gateway et 50 tests worker réussis. Le parcours complet ci-dessus, TypeScript, Expo ESLint et l’export web passent également.

Après le compte rendu Android : 124 tests API et 3 tests React couvrent le seuil prudent, la saisie de 120 caractères, le motif précis du 409 et la conservation du brouillon lors d’un conflit. L’intégration couvre aussi une modification bloquée conservée, le refus d’utiliser l’ancien identifiant, puis la correction réussie sur la nouvelle version. TypeScript et Expo ESLint passent.

Le parcours a été rejoué sur l’émulateur Android : proposition bloquée → saisie rapide d’une promesse commerciale → 409 avec nouveau motif affiché → correction neutre → version 3 APPROVED, sans erreur React capturée et sans envoi Meta. Le brouillon est désormais dérivé de la réponse serveur jusqu’à sa première modification ; il reste local lors d’un rafraîchissement après conflit. [Capture après validation](validation/rag-editor-approved.png). L’import de fichiers via le sélecteur natif reste à tester.

Le bouton **Traiter** sur l’accueil ouvre maintenant l’éditeur de réponse ; il ne marque plus le commentaire traité immédiatement. Le seed utilise des chaînes UTF-8 correctes. `node prisma/repair-demo-data.js` prévisualise les réparations exactes de Studio Vega ; `--apply` corrige uniquement les valeurs de seed corrompues encore présentes (nouvelle version du profil), et la fixture de commentaire signalée avec ses références. Le script est idempotent ; les anciennes propositions restent conservées.
